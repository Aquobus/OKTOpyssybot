import os
from misc import logger, random, Message, ParseMode
...
class NotClassmateMessageMiddleware(BaseMiddleware):
    async def __call__(
            self,
            handler: Callable[[Message, Dict[str, Any]], Awaitable[Any]] | Union[Any, str] | Any,
            event: Message,
            data: Dict[str, Any] | Union[Any, str] | Any) -> Any:
        ...
        await data["bot"].send_message(
            chat_id=-8498888674,
            text=f'Achtung! Alarm! An unknown user is trying to interact with the bot!\nUser ID: {data["event_from_user"].id}\nUSERNAME: {data["event_from_user"].username}\nFROM CHAT: {data["event_chat"].title or data["event_chat"].username}\nMESSAGE: {event.text}\n',
